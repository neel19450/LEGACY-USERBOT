...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if mention.sender_id not in USERS or chat_title not in USERS:
            if AFKREASON:
                await mention.reply(
                    f"**I'm not available right now join @LEGACY_USERBOT_SUPPORT_CHAT .** (Since: {afk_str})"
                    f"\nReason: `{AFKREASON}`."
                )
            else:
                await mention.reply(
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(
                            "**I'm not available right now join @LEGACY_USERBOT_SUPPORT_CHAT .** (Since: {afk_str})"
                            f"\nReason: `{AFKREASON}`."
                        )
                    else:
                        await sender.reply(
